...
// that can be encoded by conventional codecs such as ZMBV and XVID.
// The Windows version relies on the Video For Windows (VFW) interface;
// other systems will get RGB24+PCM AVI files for FFMPEG and others.
...
int xrf_read(void) // reads a XRF chunk and decodes the current video and audio frames; !0 ERROR/EOF
{
	...
	logchar('1'); s+=xrf_decode(&diff32[0],s,&j,4); i+=j; // B
	logchar('2'); s+=xrf_decode(&diff32[1],s,&j,4); i+=j; // G
	logchar('3'); s+=xrf_decode(&diff32[2],s,&j,4); i+=j; // R
	...
	if (audio_z)
	{
		...
		{
			case 0: // 8-bit mono
				logchar('4'); s+=xrf_decode(&wave32[0],s,&k,1); i+=k; // M
				i-=audio_z;
				break;
			case 1: // 16-bit mono
			case 2: // 8-bit stereo: equal to 16-bit mono on the lil-endian AVI!
				logchar('4'); s+=xrf_decode(&wave32[0],s,&k,2); i+=k; // m/L
				logchar('5'); s+=xrf_decode(&wave32[1],s,&k,2); i+=k; // M/R
				i-=audio_z*2;
				break;
			case 3: // 16-bit stereo
				logchar('4'); s+=xrf_decode(&wave32[0],s,&k,4); i+=k; // l
				logchar('5'); s+=xrf_decode(&wave32[1],s,&k,4); i+=k; // L
				logchar('6'); s+=xrf_decode(&wave32[2],s,&k,4); i+=k; // r
				logchar('7'); s+=xrf_decode(&wave32[3],s,&k,4); i+=k; // R
				i-=audio_z*4;
				break;
		}
